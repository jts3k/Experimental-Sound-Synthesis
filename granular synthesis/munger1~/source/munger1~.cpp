...
munger1::munger1(int argc, const t_atom *argv):maxdelay(3000.), num_channels(2), numvoices(50), munger_name(NULL) //argc: argument #, argv:  argument value
{
	float arg1;
	int i,j,n;
	int arg2, arg3;
	...
	post("munger1~ version %d.%d.%d", (int)MUNGER_MAJOR, (int)MUNGER_MINOR, (int)MUNGER_REV);
	...
	if(argc)
	{
		...
		if(argc > 1)
		{
			...
			num_channels = arg2;
			post ("munger1~: number channels = %d", num_channels);
		}
		...
		if (argc > 2)
		{
			...
			if (arg3)
			{
				...
				numvoices = arg3;
				post ("munger1~: maximum possible number of voices = %d", numvoices);
			}
		}
		if (argc > 3)
		{
	...
	verbose = 1;
	post("munger1~ %s: maxdelay = %d milliseconds", munger_name, (long)maxdelay);
...
void munger1::munger_spat(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		for (i=j=0; i < (argc - 1); i+=2)
		{
			...
			if(verbose > 1) post("munger1~ %s: channel gain %d = %f, spread = %f", munger_name, j, channelGain[j], channelGainSpread[j]);
			j++;
		}
	}
}
...
void munger1::munger_note(short argc, t_atom *argv)
{
	if (oneshot)
	{
		...
		if(argc < 8)
		{
			post("munger1~ %s: need 8 args -- transposition, gain, pan, attkT, decayT, susLevel, relT, direction [-1/1]", munger_name);
			return;
		}
		...
		if(newnote > voices)
		{
			if(verbose > 0) post("munger1~ %s: too many notes amadeus.", munger_name);
			return;
		}
...
void munger1::munger_oneshot(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		temp = GetAInt(argv[0]);
		oneshot = temp;
    		if(verbose > 1) post("munger1~ %s: setting oneshot: %d", munger_name, temp);
	}
}


//external buffer stuff
bool munger1::munger_checkbuffer(bool reset)
{
    if (!l_buf) {
    //if(!l_buf || !l_buf->Ok() || !l_buf->Valid()) {
        post("munger1~ %s: error: no valid buffer defined", munger_name);
        // return zero length
        return false;
    }
    ...
    if (!l_buf->Ok())
    {
		post("munger1~ %s: buffer mysteriously dissapeared. Reverting to internal buffer...", munger_name);
		munger_clearbuffer();
		return false;
    }
    else if (l_buf->Update())
    {
            ...
            if(l_buf->Valid()) {
                if(verbose > 1) post("munger1~ %s: updated buffer reference", munger_name);
                return true;
            }
            else {
					post("munger1~ %s: error: buffer has become invalid", munger_name);
					munger_clearbuffer();
					return false;
            }
    }
...
void munger1::munger_clearbuffer()
{
	if (l_buf)
	{
		...
		bufname = NULL;
		externalBuffer = 0;
		if(verbose > 1) post("munger1~ %s: external buffer deleted.", munger_name);
	}
}
...
void munger1::munger_setbuffer(short argc, t_atom *argv)
{
	if(argc == 0) {
		...
		if (l_buf) munger_clearbuffer();
        	...
        	if(!l_buf->Ok()) {
            		if(verbose > 0) post("munger1~ %s: error: buffer %s is currently not valid!", munger_name, GetAString(argv[0]));
        	}
		else
		{
			if(verbose > 1) post("munger1~ %s: successfully associated with the %s buffer.", munger_name, GetAString(argv[0]));
			externalBuffer = 1;
			l_chan = 0;
		}
    	}
        	...
        	if(verbose > 0) post("munger1~ %s: error: message argument must be a string.", munger_name);
		if (l_buf) munger_clearbuffer();
    	}
}
...
void munger1::setverbose(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if (temp > 3) temp = 3;
		verbose = temp;
		post("munger1~ %s: setting verbose: %d", munger_name, temp);
		if (verbose < 3)
		{
			graincounter = 0;
			countsamples = 0;
...
float munger1::getExternalSamp(double where)
{
	...
	if (!b || !b->Ok()) {
		post("munger1~ %s: error: external buffer mysteriously AWOL, reverting to internatl buffer...", munger_name);
		externalBuffer = 0;
		return 0.;
	}
...
void munger1::munger_setramp(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if (verbose > 1) post("munger1~ %s: setting ramp to: %f ms", munger_name, (rampLength * one_over_srate_ms));
	}
}
...
void munger1::munger_scale(short argc, t_atom *argv)
{
	...
	if (verbose > 1) post("munger1~ %s: loading scale from input list", munger_name);
	smoothPitch = 0;
...
void munger1::munger_bufsize(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if(temp < 20.*(float)MINSIZE)
		{
			...
			if (verbose > 0) post("munger1~ %s error: delaylength too small!", munger_name);
		}
		//if(temp > (float)BUFLENGTH) temp = (float)BUFLENGTH;
		if(temp > initbuflen)
		{
			temp = initbuflen;
			if (verbose > 0) post("munger1~ %s error: delaylength too large!", munger_name);
		}
		//buflen = temp;
		//maxsize = buflen / 3.;
		maxsize = temp / 3.;
    		twothirdBufsize = maxsize * 2.;
    		onethirdBufsize = maxsize; 
    		if (verbose > 1) post("munger1~ %s: setting delaylength to: %f seconds", munger_name, temp/srate);
...
void munger1::munger_bufsize_ms(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		{
			temp = 20.*(float)MINSIZE;
			if (verbose > 0) post("munger1~ %s error: delaylength_ms too small!", munger_name);
		}
		//if(temp > (float)BUFLENGTH) temp = (float)BUFLENGTH;
		if(temp > initbuflen)
		{
			temp = initbuflen;
			if (verbose > 0) post("munger1~ %s error: delaylength_ms too large!", munger_name);
		}
		//buflen = temp;
		//maxsize = buflen / 3.;
		maxsize = temp / 3.;
    		twothirdBufsize = maxsize * 2.;
    		onethirdBufsize = maxsize; 
    		if (verbose > 1) post("munger1~ %s: setting delaylength to: %d milliseconds", munger_name, (long)(temp/srate_ms));
	}
}
void munger1::munger_setminsize(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if(temp >= initbuflen) {
			temp = (float)MINSIZE;
			if (verbose > 0) post("munger1~ %s error: minsize too large!", munger_name);
		}
		minsize = temp;
    		if (verbose > 1) post("munger1~ %s: setting min grain size to: %f ms", munger_name, (minsize/srate_ms));
...
void munger1::munger_discretepan(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if(temp < 0)
		{
			if (verbose > 0) post("munger1~ %s error: discretepan can be only 0 or 1!", munger_name);
			temp = 0;
		}
		if(temp > 1)
		{
			if (verbose > 0) post("munger1~ %s error: error: discretepan can be only 0 or 1!", munger_name);
			temp = 1;
		}
		discretepan = temp;
...
void munger1::munger_setvoices(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if(temp < 0)
		{
			if (verbose > 0) post("munger1~ %s error: voices has to be between 0 and maxvoices (currently %d)!", munger_name, maxvoices);
			temp = 0;
		}
		if(temp > maxvoices)
		{
		...
		voices = temp;
    		if (verbose > 1) post("munger1~ %s: setting voices to: %d ", munger_name, voices);
...
void munger1::munger_maxvoices(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if(temp < 0)
		{
			if (verbose > 0) post("munger1~ %s error: maxvoices cannot be less than 0!", munger_name);
			temp = 0;
		}
		if(temp > numvoices) temp = numvoices;
		maxvoices = temp;
    		if (verbose > 1) post("munger1~ %s: setting max voices to: %d ", munger_name, maxvoices);
	}
}
...
void munger1::setpower(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		temp = GetAInt(argv[0]);
		power = temp;
    		post("munger1~ %s: setting power: %d", munger_name, temp);
	}
}
void munger1::munger_record(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		else
		{
    		...
    		if (verbose > 1) post("munger1~ %s: setting record: %d", munger_name, temp);
	}
}
...
void munger1::munger_ambidirectional(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		temp = GetAInt(argv[0]);
		ambi = temp;
    		if (verbose > 1) post("munger1~ %s: setting ambidirectional: %d", munger_name, temp);
	}
}
...
void munger1::munger_gain(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		temp = GetAFloat(argv[0]);
    		if (verbose > 1) post("munger1~ %s: setting gain to: %f ", munger_name, temp);
    		gain = temp;
	}
}
...
void munger1::munger_setposition(short argc, t_atom *argv)
{
	...
	if (argc)
	{
		...
		if (temp > 1.) temp = 1.;
    		if (temp < 0.) temp = -1.;
    		if (verbose > 1) post("munger1~ %s: setting position to: %f ", munger_name, temp);
    		position = temp;
	}
}
...
void munger1::munger_randgain(short argc, t_atom *argv)
{
	...
	{
		temp = GetAFloat(argv[0]);
    		if (verbose > 1) post("munger1~ %s: setting rand_gain to: %f ", munger_name, temp);
    		randgain = temp;
	}
}
...
{
	if (verbose > 1) post("munger1~ %s: hanning window is busted", munger_name);
	doHanning = 1;
}
...
{
	int i;
	if (verbose > 1) post("munger1~ %s: doing tempered scale", munger_name);
	smoothPitch = 0;
	scale_len = 100;
...
{
	if (verbose > 1) post("munger1~ %s: doing smooth scale", munger_name);
	smoothPitch = 1;
}
...
void munger1::munger_alloc()
{
	...
	if (!recordBuf)
	{
		error("munger1~ %s: out of memory", munger_name);
	}
	...
	if(!power) {
		...
		while(n--) {
			if (verbose > 2)
			{
				...
				{
					post("munger1~ %s: outputting %d grains per second", munger_name, graincounter);
					graincounter = 0;
					countsamples = 0;
				}
			}
...
{
	post (">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	post ("***CURRENT munger1~ %s PARAMETER VALUES***:", munger_name);
